...
WebElement Date=driver.findElement(By.xpath("//select[@aria-label='Select month']"));
Select sec=new Select(Date);
sec.selectByVisibleText("Jul");
WebElement Year=driver.findElement(By.xpath("//select[@aria-label='Select year']"));
Select sec1=new Select(Year);
sec1.selectByVisibleText("2023");
List<WebElement> listall=driver.findElements(By.xpath("//table[@class='ui-datepicker-calendar']//td"));

for(WebElement uio:listall) {
	
	String oio=uio.getText();
if(oio.equals("15")) {
